...
# =========================
# DEBUG (validação de imagens)
# =========================
DEBUG_IMAGES = False  # coloque True para imprimir logs de validação das imagens
...
_CDN_PRODUCT_ID_RE = re.compile(r"/produto/(\d+)(?:/|$)", re.I)
...
def normalize_image_url(url: str) -> str:
    ...
    # (mantive o mesmo comportamento/estrutura do seu código)
    if "cdn.awsl" in url:
...
def extract_cdn_product_id_from_image(url: str) -> Optional[str]:
    """
    Extrai o ID numérico do produto a partir da URL do CDN:
    https://cdn.awsli.com.br/.../produto/372827353/arquivo.webp -> "372827353"
    """
    url = normalize_url(url)
    if not url:
        return None
    m = _CDN_PRODUCT_ID_RE.search(url)
...
def extract_gallery_images(product_url: str, expected_cdn_product_id: Optional[str] = None) -> List[str]:
    """
    Tenta extrair imagens da galeria da página do produto.
    Estratégia original (mantida):
    ...
    ✅ Correção (para não misturar imagens):
    - se expected_cdn_product_id existir, só aceita URLs que contenham /produto/<ID>/
    """
    html_text = http_get(product_url)
    soup = BeautifulSoup(html_text, "lxml")
    ...
    def should_accept(u: str) -> bool:
        nonlocal accepted, rejected, rejected_examples
        ...
        # ✅ filtro forte por ID do produto (evita mistura)
        if expected_cdn_product_id:
            if f"/produto/{expected_cdn_product_id}/" not in u:
                rejected += 1
                if DEBUG_IMAGES and len(rejected_examples) < 5:
                    rejected_examples.append(u)
                return False
    ...
    out = sorted(urls)[:MAX_ADDITIONAL_IMAGES]

    if DEBUG_IMAGES:
        pid = expected_cdn_product_id or "N/A"
        print(f"[DEBUG][IMGS] PID={pid} | aceitas={accepted} | rejeitadas={rejected} | finais={len(out)}")
        if rejected_examples:
            print("[DEBUG][IMGS] Exemplos rejeitados (provável outro produto):")
...
def enrich_items(base_items: List[dict]) -> List[dict]:
    ...
    ✅ Correção aplicada:
    - usa o ID do produto na URL do CDN da imagem principal para filtrar as adicionais,
      evitando misturar imagens de outros produtos.
    """
    enriched = []
    ...
    for idx, d in enumerate(base_items, start=1):
        ...
        # ✅ ID do produto a partir da imagem principal (chave para não misturar)
        expected_cdn_product_id = extract_cdn_product_id_from_image(d.get("image_link", ""))

        if DEBUG_IMAGES:
            print(f"[DEBUG][IMGS] ({idx}/{total}) link={link}")
            print(f"[DEBUG][IMGS] image_link={d.get('image_link','')}")
            print(f"[DEBUG][IMGS] expected_cdn_product_id={expected_cdn_product_id}")

        # Galeria
        try:
            gallery = extract_gallery_images(link, expected_cdn_product_id=expected_cdn_product_id)
        except Exception as e:
            gallery = []
